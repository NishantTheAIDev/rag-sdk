...
class DenseRetriever(Retriever):
    ...
    def add_chunks(self, chunks: Sequence[Chunk]) -> None:
        ...
        metadata = {chunk.id: chunk.metadata.model_dump() for chunk in chunks}
        self._store.add(ids, vectors, metadata)
        self._chunks.update({chunk.id: chunk for chunk in chunks})

    def search(self, query: str, top_k: int) -> list[RetrievalResult]:
        query_vector = self._embed([query])[0]
        filters = self._config.filters if self._config else None
        hits = self._store.search(query_vector, top_k, filters)
        return [
