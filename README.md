...
rag evaluate results.jsonl --k 10       # score retrieval results (JSONL)
rag evaluate-pipeline rag.yaml -d queries.jsonl   # end-to-end: retrieval + answers
rag experiment rag.yaml                 # parameter sweep
rag optimize rag.yaml                   # Pareto optimization -> runs/optimized-rag.yaml
rag export-config runs/optimized-rag.yaml -f json   # convert a config to JSON
```
...
```bash
rag experiment rag.yaml    # writes results.csv/json, leaderboard.csv, report.html
rag optimize rag.yaml      # writes the best full config to runs/optimized-rag.yaml
```

Strategy sweeps keep shared settings such as `top_k`, `candidate_k`,
`chunk_size` and `overlap`, and metrics are scored at document level by
default (`experiments.relevance_level`), so chunk sizes compare fairly. See
`configs/experiment.yaml` for a complete example and `docs/index.md` for the
